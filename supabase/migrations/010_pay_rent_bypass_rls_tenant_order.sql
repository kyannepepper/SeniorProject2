-- =============================================================================
-- Fix pay_rent_and_create_next_payment: RLS can still block reads/writes inside
-- SECURITY DEFINER in some Supabase setups. Disable row_security for this
-- trusted RPC after auth.uid() checks. Prefer a tenant row that has a lease.
-- =============================================================================
...
DECLARE
  ...
  v_next_due date;
BEGIN
  -- Let this function read/update payments and tenants regardless of RLS (caller is still scoped by auth.uid())
  ...
  -- Prefer a tenant row that has a lease (stable if duplicate rows ever exist)
